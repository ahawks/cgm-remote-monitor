...
function init(env, ctx) {
  ...
  data.calcSigs = function calcSigs () {

    var sigs = {};

    var typeKeyFields = {
      'sgvs': ['x', 'y', 'direction', 'filtered', 'unfiltered', 'device']
      , 'treatments': ['created_at', 'eventType', 'glucose', 'glucoseType', 'carbs', 'insulin', 'enteredBy']
      , 'mbgs': ['x', 'y', 'device']
      , 'cals': ['x', 'y', 'scale', 'slope', 'intercept', 'device']
      , 'profiles': ['dia', 'sens', 'carbratio', 'carbs_hr', 'target_high', 'target_low']
      , 'devicestatus': ['uploaderBattery']
    };

    _.forIn(typeKeyFields, function eachPair (keyFields, type) {
      var hash = crypto.createHash('sha1');
      _.forEach(data[type], function eachValue(value) {
        _.forEach(keyFields, function eachField (field) {
          var fieldValue = value[field] && value[field].toString();
          if (fieldValue) {
            hash.update(fieldValue)
          }
        });
      });
      sigs[type] = hash.digest('base64');
    });
